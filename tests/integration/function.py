...
def compare_energy_values(energy1, energy2, isPrint=True, isReturn=False):
    ...
    if relative_energy_error <= 1e-5 or abs_energy_error <= 1e-3:
        ...
        if isReturn:
            return (True, abs_energy_error, relative_energy_error)
        else:
            return True
    else:
        if isPrint:
        ...
        if isReturn:
            return (False, abs_energy_error, relative_energy_error)
        else:
            return False
...
def compare_forces_values(forces1, forces2, isPrint=True, isReturn=False):
    ...
    if max_allclose <= 0:
        if isPrint:
            print("Forces match!")
        if isReturn:
            return (True, max_allclose, abs_force_error, relative_force_error)
        else:
            return True
    else:
        if isPrint:
