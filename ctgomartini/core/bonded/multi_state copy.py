...
# Legacy classes omitted for brevity - same as before
@register_interaction
class MultiHarmonicAngles(Interaction):
    class Idx:
        ATOM1, ATOM2, ATOM3, NSTATES, STATEID, FUNCTYPE, ANGLE, K = range(8)
    EXPECTED_FIELDS = 8

    def __init__(self) -> None:
        super().__init__(
            name='multi_harmonic_angles',
            description='Multi_harmonic angle potential (legacy)',
            category='multi_angles',
    ...
    def add_interaction(self, state: str, fields: list[str], base_atom_index: int = 0, offset: int = -1) -> None:
        self._validate_field_count(fields, self.EXPECTED_FIELDS)
        self._validate_functype(fields)
        idx = self.Idx
        if fields[idx.STATEID] != state:
            raise MultiStateError(state, self.category, fields, f"State mismatch")
...
@register_interaction
class MultiG96Angles(Interaction):
    class Idx:
        ATOM1, ATOM2, ATOM3, NSTATES, STATEID, FUNCTYPE, ANGLE, K = range(8)
    EXPECTED_FIELDS = 8
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            description='multi_g96 angles (legacy)',
            category='multi_angles',
            mm_force=mm.CustomAngleForce("0.5 * k * (cos(theta) - cos(theta0))^2"),
            type_label=[5, "2"],
        )
...
@register_interaction
class MultiRestrictedAngles(Interaction):
    class Idx:
        ATOM1, ATOM2, ATOM3, NSTATES, STATEID, FUNCTYPE, ANGLE, K = range(8)
    EXPECTED_FIELDS = 8
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            description='Multi_restricted angles (legacy)',
            category='multi_angles',
            mm_force=mm.CustomAngleForce("0.5 * k * (cos(theta) - cos(theta0))^2 / sin(theta)^2"),
            type_label=[5, "10"],
        )
        self.mm_force.addPerAngleParameter('theta0')
        self.mm_force.addPerAngleParameter('k')
...
@register_interaction
class MultiPeriodicDihedrals(Interaction):
    class Idx:
        ATOM1, ATOM2, ATOM3, ATOM4, NSTATES, STATEID, FUNCTYPE, THETA, K, PHASE = range(10)
    EXPECTED_FIELDS = 10

    def __init__(self) -> None:
        super().__init__(
            name='multi_periodic_dihedrals',
            description='Multi_periodic dihedrals (legacy)',
            category='multi_dihedrals',
...
@register_interaction
class MultiHarmonicDihedrals(Interaction):
    class Idx:
        ATOM1, ATOM2, ATOM3, ATOM4, NSTATES, STATEID, FUNCTYPE, THETA, K = range(9)
    EXPECTED_FIELDS = 9

    def __init__(self) -> None:
        super().__init__(
            name='multi_harmonic_dihedrals',
            description='Multi_harmonic dihedrals (legacy)',
            category='multi_dihedrals',
            mm_force=mm.CustomTorsionForce(
                "0.5*k*(thetap-theta0)^2; thetap = step(-(theta-theta0+pi))*2*pi+theta+step(theta-theta0-pi)*(-2*pi); pi = %.15g" % math.pi
            ),
            type_label=[6, "2"],
        )
...
@register_interaction
class MultiContacts(Interaction):
    class Idx:
    ...
    def __init__(
        ...
        super().__init__(
            name='multi_contacts',
            description='Multi_contacts (legacy)',
            category='multi_contacts',
            mm_force=None,
            type_label=[4, "1"],
        )
...
__all__ = [
    ...
    "MultiHarmonicAngles",
    "MultiG96Angles",
    "MultiRestrictedAngles",
    "MultiPeriodicDihedrals",
    "MultiHarmonicDihedrals",
    "MultiContacts",
]
