...
def MBGOMartinize(aa_strfile_list, aa_map_list, state_name_list, mbmol_name, dict_cutoffs, method='exp', dssp='dssp', ff='martini3001', other_params=''):
...
def SBGOMartinize(aa_strfile_list, aa_map_list, state_name_list, sbmol_name, method='SBP', dssp='dssp', ff='martini3001', other_params=''):
    working_path = os.getcwd()
    print(f'Working path: {working_path}')
    ...
    for aa_strfile, aa_map, state_name in zip(aa_strfile_list, aa_map_list, state_name_list):
        ...
        print(f'\nMartinize the all-atom protein ({aa_strfile}) as the CG model with the state name ({state_name})')
        Martinize2(os.path.join('../', aa_strfile), dssp, ff, state_name, other_params=f'-scfix {other_params}')
...
def SwitchingGOMartinize(aa_strfile_list, aa_map_list, state_name_list, mbmol_name, dict_cutoffs, method='switching', dssp='dssp', ff='martini3001', other_params=''):
    working_path = os.getcwd()
    print(f'Working path: {working_path}')
...
def CTGOMartinize(aa_strfile_list, aa_map_list, state_name_list, mbmol_name, dict_cutoffs, method='switching', dssp='dssp', ff='martini3001', other_params=''):
    if method.lower() == 'switching':
        SwitchingGOMartinize(aa_strfile_list, aa_map_list, state_name_list, mbmol_name, dict_cutoffs, method=method, dssp=dssp, ff=ff, other_params=other_params)
    elif method.lower() in ['exp', 'ham']:
        MBGOMartinize(aa_strfile_list, aa_map_list, state_name_list, mbmol_name, dict_cutoffs, method=method, dssp=dssp, ff=ff, other_params=other_params)
    elif method.lower() == 'sbp':
        SBGOMartinize(aa_strfile_list, aa_map_list, state_name_list, sbmol_name=mbmol_name, method=method, dssp=dssp, ff=ff, other_params=other_params)
...
""")
    ...
    CTGOMartinize(args.strfile, args.mapfile, args.moltype, args.mbmoltype, dict_cutoffs, method=args.method, dssp=args.dssp, ff=args.ff, other_params=args.other_params)
