...
#: Bug: extract is not good for CA
...
def user_input(args_string: str) -> Namespace:
    """Parse command line arguments for virtual site generation.
...
def get_settings() -> tuple[str, str, str, str, int, list[int], int]:
    """Get default settings for temporary files and parameters.

    Returns:
        A tuple containing:
            - file_CA: Name for temporary CA atom file.
            - file_BB: Name for temporary BB bead file.
            - file_OV: Name for temporary OV contact map file.
            - file_rCSU: Name for temporary rCSU contact map file.
            - seqDist: Minimal sequence distance for elastic bonds.
            - cols: Column indices of interest in contact map files.
            - c6c12: Flag for C6/C12 terms vs sigma/go_eps format.
    """
...
def read_data(
    ...
    file_rCSU: str,
    cols: list[int],
) -> tuple[NDArray[np.float64], dict[tuple[str, int], list[int]], list[str], list[list]] | None:
    """Read and process input data from PDB and contact map files.

    Args:
        struct_pdb: Path to coarse-grained structure PDB file.
        ref_pdb: Path to all-atom reference PDB file.
        file_contacts: Path to contact analysis file.
        file_CA: Temporary file name for CA atoms.
        file_BB: Temporary file name for BB beads.
        file_OV: Temporary file name for OV contacts.
        file_rCSU: Temporary file name for rCSU contacts.
        cols: Column indices to extract from contact map files.

    Returns:
        A tuple containing:
            - indBB: NumPy array of BB bead data (newresid, newatomid, px, py, pz).
            - order: Dictionary mapping (chainid, resid) to [index_resid, newresid].
            - nameAA: List of residue names.
            - map_OVrCSU: List of contact map entries.
        Returns None if reference and CG PDB files have mismatched lengths.
    """
    ...
    indBB: list[list[float]] = []  # newresid, newatomid, position x,y,z
    nameAA: list[str] = []  # residue name
    ...
    indBB_arr = np.array(indBB) # newresid, newatomid, px, py, pz

    # read file_CA 
    with open(file_CA,'r') as fid:
        dat=fid.readlines()
    dat = dat[:-1]
    order: dict[tuple[str, int], list[int]] = {}  # (chainid, resid):[index_resid, newresid]
    ...
    for k in range(0, len(dat)):
        ...
        resid = int(tmp[22:26])
        key = (chainid, resid)
        value = [k, indBB_arr[k,0]] # considered that the orders of CG file and ref pdbfile are same
        order[key] = value
    ...
    return indBB_arr, order, nameAA, map_OVrCSU


def get_go(
    indBB: NDArray[np.float64],
    ...
    """Calculate Go-like contacts from processed data.

    Args:
        indBB: NumPy array of BB bead coordinates and indices.
        order: Dictionary mapping (chainid, resid) to [index_resid, newresid].
        nameAA: List of residue names (unused but kept for compatibility).
        map_OVrCSU: List of contact map entries from OV and rCSU analysis.
        cutoff_short: Lower cutoff distance in nm for contacts.
        cutoff_long: Upper cutoff distance in nm for contacts.
        go_eps: Dissociation energy for Lennard-Jones potential in kJ/mol.
        seqDist: Minimal sequence distance for considering contacts.

    Returns:
        List of symmetric contact pairs with calculated parameters.
        Each pair contains: [newresid1, newatomid1, newresid2, newatomid2,
                            Vii, Wii, contact_distance, sigma].
    """
    ...
    for k in range(0, len(map_OVrCSU)):
        if (map_OVrCSU[k][4] > cutoff_short) and (map_OVrCSU[k][4] < cutoff_long) and \
           ((map_OVrCSU[k][0] != map_OVrCSU[k][2]) or (abs(map_OVrCSU[k][1]-map_OVrCSU[k][3]) >= seqDist)):
            ...
            #pairs: newresid 1, newatomid 1, newresid 2, newatomid 2, Vii, Wii, contact distance, sigma
...
def write_files(
    ...
    """Write output topology files for Go-like virtual site interactions.

    Args:
        file_pref: Prefix for output file names (typically molecule type).
        sym_pairs: List of symmetric contact pairs with parameters.
        order: Dictionary mapping (chainid, resid) to [index_resid, newresid].
        indBB: NumPy array of BB bead coordinates and indices.
        Natoms: Number of coarse-grained beads excluding virtual Go beads.
        nameAA: List of residue names (unused but kept for compatibility).
        go_eps: Dissociation energy for Lennard-Jones potential in kJ/mol.
        c6c12: Flag for C6/C12 terms (1) vs sigma/go_eps format (0).
    """
...
def create_go_virt_for_multimer(args_string: str) -> None:
    """Generate virtual sites for multimeric proteins with Go-like interactions.

    This is the main entry point for processing multimeric protein structures
    and generating virtual site definitions with Go-like contact interactions.

    Args:
        args_string: String containing command line arguments. Expected flags:
            -s: Coarse-grained structure PDB file.
            -r: All-atom reference PDB file.
            -f: Contact analysis file from rCSU webserver.
            --moltype: Molecule name prefix (default: molecule_0).
            --go_eps: Dissociation energy for LJ potential (default: 9.414).
            --cutoff_short: Lower cutoff distance in nm (default: 0.3).
            --cutoff_long: Upper cutoff distance in nm (default: 1.1).
            --Natoms: Number of CG beads excluding virtual Go beads.

    Example:
        >>> create_go_virt_for_multimer(
        ...     "-r protein.pdb -s protein_cg.pdb -f protein.map "
        ...     "--moltype pro --go_eps 12 --Natoms 100"
        ... )
    """
    args = user_input(args_string)

    file_CA, file_BB, file_OV, file_rCSU, seqDist, cols, c6c12 = get_settings()

    result = read_data(args.s, args.r, args.f, file_CA, file_BB, file_OV, file_rCSU, cols)
    if result is None:
        return
    indBB, order, nameAA, map_OVrCSU = result
