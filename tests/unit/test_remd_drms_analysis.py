...
class TestPositionExtractor:
    """Tests for PositionExtractor class."""
    
    @pytest.fixture
    def netcdf_files(self):
        """Provide NetCDF file paths."""
        return {
            'nc': str(FIXTURES_DIR / "output.nc"),
            'checkpoint': str(FIXTURES_DIR / "output_checkpoint.nc"),
            'topology': str(FIXTURES_DIR / "npt.pdb"),
        }
    
    def test_position_extractor_initialization(self, netcdf_files):
        """Test PositionExtractor can be initialized."""
        from ctgomartini.analysis.remd_drms_analysis import PositionExtractor
        
        extractor = PositionExtractor(
            netcdf_files['nc'],
            netcdf_files['checkpoint']
        )
        
        # Check metadata is loaded
        assert extractor.n_frames > 0
        assert extractor.n_replicas > 0
        assert extractor.n_atoms > 0
        assert extractor.dt > 0
        
        extractor.close()
    
    def test_get_positions_single_frame(self, netcdf_files):
        """Test getting positions for single frame."""
        from ctgomartini.analysis.remd_drms_analysis import PositionExtractor
        ...
        # Get positions for frame 0, replica 0
        positions = extractor.get_positions(0, 0)
        
        assert isinstance(positions, np.ndarray)
        assert positions.ndim == 2
        assert positions.shape[0] == extractor.n_atoms
        assert positions.shape[1] == 3  # x, y, z
        
        extractor.close()
    
    def test_get_positions_batch(self, netcdf_files):
        ...
        positions = extractor.get_positions_batch(
            frame_indices, replica_indices, atom_indices
        )
        
        assert isinstance(positions, np.ndarray)
        assert positions.shape == (2, 2, 3, 3)  # (frames, replicas, atoms, 3)
