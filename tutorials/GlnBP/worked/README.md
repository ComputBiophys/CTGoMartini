...
(2) Generate the Multiple-basin Gō-Martini model for GlnBP. Contact maps can be obtained either from the [web-server](http://info.ifpan.edu.pl/~rcsu/rcsu/index.html) with default settings, or automatically generated by `ctgomartinize` with the `-m auto` option. The Multiple-basin Martini topology files (GlnBP.itp and GlnBP_params.itp) are also generated using `ctgomartinize`

```bash
ctgomartinize -s 1GGG_clean.pdb 1WDN_clean.pdb -m 1GGG_clean.map 1WDN_clean.map -mol Open Closed -mbmol GlnBP -dssp -ff martini3001 -method exp

# or

ctgomartinize -s 1GGG_clean.pdb 1WDN_clean.pdb -m auto -mol Open Closed -mbmol GlnBP -dssp -ff martini3001 -method exp
```
...
If you prefer to use the Hamiltonian mixing scheme instead, specify `-method ham` when running ctgomartinize and then modify the parameters accordingly.

```bash
ctgomartinize -s 1GGG_clean.pdb 1WDN_clean.pdb -m auto -mol Open Closed -mbmol GlnBP -dssp -ff martini3001 -method ham
vi GlnBP.itp # Change delta, C1, and C2 to 350, -348, and 0, respectively.
```
