...
def GenGoContacts(aa_strfile, cg_strfile, aa_map, state_name, go_eps=12):
    ...
    with contextlib.redirect_stdout(f):
        with contextlib.redirect_stderr(f):
            ...
            create_go_virt_for_multimer(f'-r {aa_strfile} -s {cg_strfile} -f {aa_map} --moltype {state_name} --go_eps {go_eps} --Natoms {Natoms}')
    output = f.getvalue()

    print(f'create_go_virt_for_multimer -r {aa_strfile} -s {cg_strfile} -f {aa_map} --moltype {state_name} --go_eps {go_eps} --Natoms {Natoms}')
    output = output.split('\n')
    for line in output:
...
def MBGOMartinize(aa_strfile_list, aa_map_list, state_name_list, mbmol_name, dict_cutoffs, method='exp', dssp='dssp', ff='martini3001'):
    ...
    for aa_strfile, aa_map, state_name in zip(aa_strfile_list, aa_map_list, state_name_list):
        ...
        print('\nConvert Long/Short Elastic Bonds to LJ Interactions')
        convert_long_short_elastic_bonds(state_name, f'{state_name}_cg.pdb', convert_long_elastic_bonds=True, convert_short_elastic_bonds=False, lj_epsilon=12)
