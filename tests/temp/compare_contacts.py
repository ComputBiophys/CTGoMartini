...
文件1: RawContacts/go_nbparams.itp - 使用 molecule_X 格式, ID从1开始
...
def compare_contacts(raw_contacts: dict, initial_contacts: dict, tolerance: float = 0.001):
    ...
    print("=" * 80)
    print()
    print(f"RawContacts/go_nbparams.itp (ID偏移+4后): {len(raw_shifted)} 个contacts")
    ...
    print(f"  仅在 RawContacts 中: {len(only_in_raw)}")
    ...
    if common:
        ...
        for key in sorted(common):
            ...
            if sigma_diff > tolerance or dist_diff > tolerance:
                diff_count += 1
                print(f"  Contact {key}:")
                print(f"    RawContacts:     sigma={raw_sigma:.6f}, dist={raw_dist:.6f}, eps={raw_epsilon}")
    ...
    # 仅在RawContacts中的contacts
    if only_in_raw:
        print("-" * 80)
        print(f"仅在 RawContacts/go_nbparams.itp 中的 contacts ({len(only_in_raw)} 个):")
    ...
    # 保存详细结果到文件
    output_file = Path(__file__).parent / "contact_comparison_result.txt"
    with open(output_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("CONTACTS 比较详细结果\n")
        f.write("=" * 80 + "\n\n")
        
        f.write("文件:\n")
        f.write("  文件1: RawContacts/go_nbparams.itp\n")
        ...
        if only_in_raw:
            f.write("-" * 80 + "\n")
            f.write(f"仅在 RawContacts/go_nbparams.itp 中的 contacts:\n")
...
def main():
    ...
    file1 = base_dir / "RawContacts" / "go_nbparams.itp"
    file2 = base_dir / "InitialContacts" / "gbp_open_go-table_VirtGoSites.itp"
    
    print("=" * 80)
    print("比较两个ITP文件的contacts")
    print("=" * 80)
    print()
    print(f"文件1: {file1}")
    print(f"文件2: {file2}")
    ...
    print(f"  从 go_nbparams.itp 解析了 {len(raw_contacts)} 个contacts")
