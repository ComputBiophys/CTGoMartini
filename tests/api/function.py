...
def OMM_calStrfile(strfile, simulation, prefix='conf', isEnergy=True, isForces=True, set_vsite=True):
    """
    strfile: .gro or .pdb
    simulation: class simulation from openmm
    prefix: default: conf
    isEnergy: output energy file: energy_{prefix}.dat
    isForces: output forces file: forces_{prefix}.dat
    set_vsite: if true, set forces of vsite zeros; else, not set.
    """
...
gmx_d grompp -f {mdpfile} -c {strfile} -p {topfile} -n {indexfile} -o {prefix}.tpr -r {strfile} -maxwarn 1 -v
...
set -e
# use double precision for everything
gmx_d grompp -f {mdpfile} -c {strfile} -p {topfile} -o {prefix}.tpr -r {strfile} -maxwarn 1 -v
gmx_d mdrun -deffnm {prefix} -rerun {trjfile} -nt 1 -v
echo pot | gmx_d energy -f {prefix} -o energy_{prefix}
echo 0 | gmx_d traj -f {prefix} -s {prefix} -of forces_{prefix}
rm mdout.mdp
...
gmx grompp -f {mdpfile} -c {strfile} -p {topfile} -n {indexfile} -o {prefix}.tpr -r {strfile} -maxwarn 1 -v
