...
if TYPE_CHECKING:
    from ..api import MartiniTopFile


def Extract_contacts_from_top(top: "MartiniTopFile", molecule_name: str) -> list[list[str]]:
...
def DifferentiateAngles(angles: list[list[str]], atoms: list[list[str]]) -> tuple[list[list[str]], list[list[str]]]:
...
def DifferentiateDihedrals(dihedrals: list[list[str]], atoms: list[list[str]]) -> tuple[list[list[str]], list[list[str]], list[list[str]]]:
...
class CombineMols:
    """Static methods for combining molecular topologies from multiple states."""
