...
class REMDRunner(BaseRunner):
    ...
    def _build_systems(
        ...
        conf, box_vectors = load_structure(self.inputs.input)
        positions = conf.getPositions()
        velocities = None

        if self.inputs.ichk:
            print(f"\nLoad checkpoint file: {self.inputs.ichk}")
        ...
        for beta, C1, C2 in zip(
            ...
            system = top.create_system(
                nonbonded_cutoff=self.inputs.nonbonded_cutoff * u.nanometer,
            ...
            assert len(positions) == top.topology.getNumAtoms(), (
                f"Error: Number of atoms in {self.inputs.input} is not the same as "
                f"that from {self.inputs.topol}!"
            )

            # Add restraints
            if self.inputs.gen_rest == "yes":
                generate_restraints(
                    self.inputs.input,
                    self.inputs.atomname,
                    self.inputs.fc,
                    self.inputs.gen_rest_file,
                )
            if self.inputs.rest == "yes":
                system = add_restraints(system, self.inputs)

            # Add plumed (not supported)
            if self.inputs.plumed == "yes":
                raise ValueError("Error: Plumed is not supported!")

            # Add barostat
            if self.inputs.pcouple == "yes":
                if self.inputs.p_type == "isotropic":
                    barostat = mm.MonteCarloBarostat(
                        self.inputs.p_ref * u.bar,
                        self.inputs.temp * u.kelvin,
                        self.inputs.p_freq,
                    )
                elif self.inputs.p_type == "membrane":
                    barostat = mm.MonteCarloMembraneBarostat(
                        self.inputs.p_ref * u.bar,
                        self.inputs.p_tens * u.bar * u.nanometers,
                        self.inputs.temp * u.kelvin,
                        self.inputs.p_XYMode,
                        self.inputs.p_ZMode,
                        self.inputs.p_freq,
                    )
                else:
                    raise Exception(
                        "Unsupported barostat type: ", self.inputs.p_type
                    )
    ...
    def run(self) -> None:
        ...
        if self.inputs.nstep > 0:
            total_simulation_time = self.inputs.nstep * self.inputs.dt * u.picosecond
            simulation_time_step = self.inputs.dt * u.picosecond

            simulation_steps = int(np.floor(total_simulation_time / simulation_time_step))
            exchange_frequency = self.inputs.exc_freq
            ...
            for system in system_list:
                thermodynamic_state = openmmtools.states.ThermodynamicState(
                    system=system, temperature=self.inputs.temp * u.kelvin
                )
            ...
            if not self.inputs.const_tol:
                self.inputs.const_tol = 1e-5
            move = openmmtools.mcmc.LangevinDynamicsMove(
                timestep=simulation_time_step,
                collision_rate=self.inputs.fric_coeff / u.picosecond,
                n_steps=exchange_frequency,
                reassign_velocities=False,
                constraint_tolerance=self.inputs.const_tol,
            )
